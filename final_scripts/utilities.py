...
def visualize(metricsXY, numRows, numCols, savePath):
    matchMatrix = np.zeros(shape=(numRows, numCols))
    for slide in range(numRows):
        for frame in range(numCols):
            matchMatrix[slide][frame] = metricsXY[(slide, frame)]
            
    fig = plt.figure()
    plt.imshow(matchMatrix)
    plt.colorbar()
    plt.xticks(np.arange(0, numCols, 1.0))
...
def computeSumDiffKeypoints(pointsSlide, pointsFrame, dimSlides, dimFrame, normalize=True):
    ...
    if toReturn > -50:
        if normalize:
            return difference / len(pointsSlide)
        else:
            return difference
    else:
        return -50

def computeRDistanceDifference(pointsSlide, pointsFrame, dimSlides, dimFrame, normalize=True):
    if (len(pointsSlide) == 0 or len(pointsSlide) == 0):
        return .8
    ...
    if (normalize):
        ...
        return distance(normalizedSlides, normalizedFrame)
...
def saveMatches(templateImageKP, videoImageKP, templateImg, videoImage, good, savePath):
    MIN_MATCH_COUNT = 10
    try:
        if len(good)>10:
            src_pts = np.float32([ templateImageKP[m.queryIdx].pt for m in good ]).reshape(-1,1,2)
...
def predictARGMIN(metricArray):
    slideFramePrediction = dict()
    for slide in range(metricArray.shape[0]):
        vidFrames = list(metricArray[slide,:])
        slideFramePrediction[slide] = vidFrames.index(min(vidFrames))
    correct = 0
    labels = dict()
    assigned = dict()
    falsePositive = 0
    ...
    numClassifiedAs = dict()
    error = 0
    for s in range(metricArray.shape[0]):
        correctItemSet.add(s)
        numClassifiedAs[s] = 0

    for s in slideFramePrediction.keys():
        if slideFramePrediction[s] == s:
            correct += 1
        numClassifiedAs[slideFramePrediction[s]] += 1
        error += (slideFramePrediction[s] - s)
    for s in numClassifiedAs.keys():
        falsePositive += max(0, numClassifiedAs[s] - 1)
     
    print("Precision: " + str(correct/ (correct + falsePositive)))

    correctlyLabeled = 0
    assigned = set()
    for s in correctItemSet:
        if slideFramePrediction[s] == s and s not in assigned:
            correctlyLabeled += 1
            assigned.add(s)

    print("Recall: " + str(correctlyLabeled/len(slideFramePrediction.keys())))
    print("Avg error: " + str(error / len(slideFramePrediction.keys())))
    print("Predictions: ")
    print(slideFramePrediction)
